...
function resolveJavaTestPackage(repoPath: string, fallbackGroupId: string | undefined): string {
  const testDir = path.join(repoPath, "tests", "java");
  for (const filePath of walkFiles(testDir, [".java"])) {
...
function renderJavaTest(packageName: string, className: string, edge: GraphEdge, importedClasses: string[]): string {
  const lines: string[] = [
    `package ${packageName};`,
    "",
  ...
  if (importedClasses.length === 0) {
    lines.push(
      "    @Test",
      `    @Disabled("Nenhuma classe de ${edge.target} foi encontrada importada no código-fonte deste " +`,
      '            "repositório; nada automatizável para validar via smoke-test de classe.")',
      "    void noImportedClassesDetected() {",
      "    }",
    );
  } else {
    for (const fqcn of importedClasses) {
      ...
      lines.push(
        "    @Test",
        `    void ${methodName}() throws ClassNotFoundException {`,
...
export const javaTestGenerator: TestGenerator = {
  id: "java-junit5",
  language: "java",
  ...
  generate(sourceRepo: RepoAnalysisResult, targetRepo, edge: GraphEdge, allRepos = []): GeneratedTest | undefined {
    ...
    const importedClasses = targetGroupId
      ? findRelevantImportedClasses(sourceRepo.repoPath, targetGroupId, otherRepos, [".java"])
      : [];

    const packageName = resolveJavaTestPackage(sourceRepo.repoPath, sourceRepo.coordinates.groupId);
    const className = `${toPascalCase(edge.target)}IntegrationRegressionTest`;
    const content = renderJavaTest(packageName, className, edge, importedClasses);

    const packageDir = path.join(sourceRepo.repoPath, "tests", "java", ...packageName.split("."));
